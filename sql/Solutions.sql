...
SELECT
	...
	last_month_sale,
	ROUND(
		(CAST(cr_month_sale AS DECIMAL(10,2)) -
		 CAST(last_month_sale AS DECIMAL(10,2)))
		/
...
(
	SELECT 
		...
		COUNT(DISTINCT s.customer_id) AS total_cx,
		ROUND(
			CAST(SUM(s.total) AS DECIMAL(12,2)) /
			CAST(COUNT(DISTINCT s.customer_id) AS DECIMAL(12,2)),
		2) AS avg_sale_pr_cx
...
SELECT 
	...
	ct.avg_sale_pr_cx,
	ROUND(
		CAST(cr.estimated_rent AS DECIMAL(12,2)) /
		CAST(ct.total_cx AS DECIMAL(12,2)),
	2) AS avg_rent_per_cx
